...
public class DaoProveedores {
    ...
    public static boolean grabar(Proveedores proveedores) {
        try {
            con = cn.conectar();
            String sql = "INSERT INTO proveedores(nombres, telefono, domicilio, correo, documento_idDocumento) VALUES(?,?,?,?,?);";
    ...
    public static boolean editar(Proveedores proveedores) {
        try {
            con = cn.conectar();
            String sql = "UPDATE proveedores SET nombres = ?, telefono =?, domicilio =?, correo =?, documento_idDocumento=? WHERE idProveedores = ?";
            ps = con.prepareStatement(sql);
            ps.setString(1, proveedores.getNombres());
            ps.setString(2, proveedores.getTelefono());
            ps.setString(3, proveedores.getDomicilio());
        ...
        try (Connection con = cn.conectar(); PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    proveedor.setDomicilio(rs.getString("domicilio"));
                    proveedor.setCorreo(rs.getString("correo"));
                    proveedor.setDocumento_idDocumento(rs.getInt("documento_idDocumentol"));
    ...
    public static String obtenerNombreProveedores(int id) {
        try {
            con = cn.conectar();
            String sql = "SELECT nombreProveedores  FROM productos WHERE idProveedores=?";
            ps = con.prepareStatement(sql);
            ps.setInt(1, id);
            rs = ps.executeQuery();
            if (rs.next()) {
                return rs.getString("nombreProveedores");
            } else {
                return "--";
            }
        } catch (SQLException ex) {
